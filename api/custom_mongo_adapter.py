...
class CustomMongoAdapter(StorageAdapter):
    """
    ...
    def count(self):
        return len(list(self.client[self.database_name]["statements"].find()))
    ...
    def filter(self, **kwargs):
        ...
        total_statements = len(list(self.client[self.database_name]["statements"].find(kwargs)))

        for start_index in range(0, total_statements, page_size):
            if mongo_ordering:
                for match in self.client[self.database_name]["statements"].find(kwargs).sort(mongo_ordering).skip(start_index).limit(page_size):
                    yield self.mongo_to_object(match)
            else:
                for match in self.client[self.database_name]["statements"].find(kwargs).skip(start_index).limit(page_size):
                    yield self.mongo_to_object(match)
    ...
    def create(self, **kwargs):
        ...
        inserted = self.client[self.database_name]["statements"].insert_one(kwargs)
    ...
    def create_many(self, statements):
        ...
        self.client[self.database_name]["statements"].insert_many(create_statements)
    ...
    def update(self, statement):
        ...
        update_operation = self.client[self.database_name]["statements"].update_one(
            search_parameters,
            update_data,
            upsert=True
        )
    ...
    def get_random(self):
        ...
        statements = self.client[self.database_name]["statements"].find().limit(1).skip(random_integer)
    ...
    def remove(self, statement_text):
        ...
        """
        self.client[self.database_name]["statements"].delete_one({'text': statement_text})
